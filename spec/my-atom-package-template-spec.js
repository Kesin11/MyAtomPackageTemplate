...
describe('MyAtomPackageTemplate', () => {
  ...
  describe('when the my-atom-package-template:toggle event is triggered', () => {
    it('hides and shows the modal panel', () => {
      ...
      // has been created
      expect(workspaceElement.querySelector('.my-atom-package-template')).not.toExist();
      ...
      runs(() => {
        expect(workspaceElement.querySelector('.my-atom-package-template')).toExist();

        let myAtomPackageTemplateElement = workspaceElement.querySelector('.my-atom-package-template');
        expect(myAtomPackageTemplateElement).toExist();

        let myAtomPackageTemplatePanel = atom.workspace.panelForItem(myAtomPackageTemplateElement);
        expect(myAtomPackageTemplatePanel.isVisible()).toBe(true);
        atom.commands.dispatch(workspaceElement, 'my-atom-package-template:toggle');
        expect(myAtomPackageTemplatePanel.isVisible()).toBe(false);
      });
    });
    ...
    it('hides and shows the view', () => {
      ...
      runs(() => {
        ...
        let myAtomPackageTemplateElement = workspaceElement.querySelector('.my-atom-package-template');
        expect(myAtomPackageTemplateElement).toBeVisible();
        atom.commands.dispatch(workspaceElement, 'my-atom-package-template:toggle');
        expect(myAtomPackageTemplateElement).not.toBeVisible();
      });
    });
  });
});
